...
public class MezclaD {
    public int tamañoArchivo(String F) {
        ...
        try {
            ...
            Logger.getLogger(MezclaD.class.getName()).log(Level.SEVERE, null, ex);
        }

        return cont;
    }

    public void MezclaDirecta(String F) {
        int n = tamañoArchivo(F);
        int part = 1;
        while (part < n) {
            particionar(F, "F1.txt", "F2.txt", part);
            fusionar(F, "F1.txt", "F2.txt", part);
            part *= 2; //part = part *2;
        }
    }
